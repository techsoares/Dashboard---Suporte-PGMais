...
Fornece:
- Validação de entrada (JIRA keys, emails, strings seguras)
- Rate limiting (por IP/endpoint, configurável)
- Sanitização de dados (remove campos sensíveis)
- Headers de segurança (CORS, CSP, HSTS)
- Logging de segurança (auditoria de eventos suspeitos)
...
# Instâncias globais com limites configuráveis via settings
api_limiter = RateLimiter(
    max_requests=settings.rate_limit_requests,
    window_seconds=settings.rate_limit_window_minutes * 60
)
auth_limiter = RateLimiter(
    max_requests=settings.auth_limit_requests,
    window_seconds=settings.auth_limit_window_minutes * 60
)
...
class InputValidator:
    ...
    def validate_list_param(items: list, max_items: int = 50) -> bool:
        ...
        if len(items) > max_items:
            logger.warning(
                f"List parameter exceeds max items: {len(items)} > {max_items}"
            )
            return False
        return True
    ...
    def sanitize_dict(data: dict) -> dict:
        ...
        for key, value in data.items():
            ...
            if any(
                ...
                for sensitive in DataSanitizer.SENSITIVE_FIELDS
            ):
                ...
                sanitized[key] = [
                    DataSanitizer.sanitize_dict(item)
...
def require_rate_limit(limiter=None):
    """
    Decorador que aplica rate limiting a uma função.

    Args:
        limiter: Instância de RateLimiter a usar (default: api_limiter)
    """
    if limiter is None:
        limiter = api_limiter
